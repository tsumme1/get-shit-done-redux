...
References for the Antigravity 2.0 enforcement layer.

| Reference | Role |
|-----------|------|
| `antigravity-runtime.md` | Translation table mapping Claude Code primitives to Antigravity tools (invoke_subagent, call_mcp_tool, ask_question, view_file). |
| `antigravity-agents.md` | Registry of 33 agent types with `define_subagent` specs for Antigravity orchestration. |

> **Subdirectory:** `get-shit-done/references/few-shot-examples/` contains additional few-shot examples (`plan-checker.md`, `verifier.md`) that are referenced from specific agents. These are not counted in the 64 top-level references.
