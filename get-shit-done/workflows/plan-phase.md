...
<antigravity_runtime>Read `get-shit-done/references/antigravity-runtime.md` FIRST. `Agent()` → `invoke_subagent` (MANDATORY), `AskUserQuestion` → `ask_question`, `gsd_run query` → `call_mcp_tool("gsd-guardian", ...)`. Spawn: researcher/pattern-mapper with `Workspace: "branch"`, planner/checker with `Workspace: "inherit"`. Enforce: `bash get-shit-done/scripts/enforce-subagent-spawn.sh "<type>"` after each spawn. MCP ONLY — raw gsd-tools.cjs PROHIBITED. Revision loop (max 3) must ALL use `invoke_subagent`.</antigravity_runtime>
