...
2. **Subagent spawning:**
   - Use `define_subagent` to register agent types (gsd-project-researcher, gsd-research-synthesizer, gsd-roadmapper)
   - Use `invoke_subagent` with `Workspace: "branch"` for parallel research agents
   - Use `invoke_subagent` with multiple Subagents entries for parallel execution (4 researchers)
